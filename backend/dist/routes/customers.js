...
router.get('/:id', async (req, res) => {
    try {
        const c = await (0, db_1.dbGet)('SELECT * FROM customers WHERE id = $id', { $id: req.params.id });
        if (!c) {
            res.status(404).json({ error: 'Not found' });
            return;
        }
        const docks = await (0, db_1.dbAll)('SELECT * FROM docks WHERE customer_id = $id ORDER BY created_at DESC', { $id: req.params.id });
        const jobs = await (0, db_1.dbAll)('SELECT * FROM jobs WHERE customer_id = $id ORDER BY scheduled_date DESC', { $id: req.params.id });
...
router.post('/', async (req, res) => {
    try {
        ...
        await (0, db_1.dbRun)('INSERT INTO customers (id,name,phone,email,address,lat,lng,notes,created_at) VALUES ($id,$name,$phone,$email,$address,$lat,$lng,$notes,$ca)', {
        ...
        const row = await (0, db_1.dbGet)('SELECT * FROM customers WHERE id = $id', { $id: id });
...
router.put('/:id', async (req, res) => {
    try {
        const { name, address, phone, email, lat, lng, notes } = req.body;
        const existing = await (0, db_1.dbGet)('SELECT * FROM customers WHERE id = $id', { $id: req.params.id });
        if (!existing) {
            res.status(404).json({ error: 'Not found' });
            return;
        }
        await (0, db_1.dbRun)('UPDATE customers SET name = COALESCE($name,name), address = COALESCE($address,address), phone = $phone, email = $email, lat = $lat, lng = $lng, notes = $notes WHERE id = $id', {
        ...
        const row = await (0, db_1.dbGet)('SELECT * FROM customers WHERE id = $id', { $id: req.params.id });
...
router.delete('/:id', async (req, res) => {
    try {
        const result = await (0, db_1.dbRun)('DELETE FROM customers WHERE id = $id', { $id: req.params.id });
        if (result.changes === 0) {
            res.status(404).json({ error: 'Not found' });
            return;
        }
