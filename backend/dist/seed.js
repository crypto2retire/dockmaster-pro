...
async function seed() {
    ...
    const existing = await (0, db_1.dbGet)('SELECT COUNT(*) as c FROM customers', {});
    if (existing.c > 0) {
    ...
    for (const c of customers) {
        await (0, db_1.dbRun)('INSERT INTO customers (id,name,phone,email,address,lat,lng,notes,created_at) VALUES ($id,$n,$p,$e,$a,$lat,$lng,$notes,$ca)', {
    ...
    // Jobs
    const now = new Date();
    const d = (o) => { const x = new Date(now); x.setDate(x.getDate() + o); return x.toISOString().split('T')[0]; };
    const jobs = [
    ...
    for (const j of jobs) {
        const id = genId('job_');
        await (0, db_1.dbRun)('INSERT INTO jobs (id,customer_id,dock_id,type,scheduled_date,crew,notes,waterway_access,previous_year_notes,status,priority,created_at,completed_at) VALUES ($id,$cid,$did,$type,$sd,$crew,$notes,$wa,$pyn,$st,$pr,$ca,$cc)', {
